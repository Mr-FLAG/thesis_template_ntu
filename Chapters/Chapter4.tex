\chapter{Task Scheduling and Transmission for Distributed Job Execution\protect\footnote{This chapter is based on our paper published in IEEE ICDCS 2026.}}
\label{ch:WAST}
In this chapter, we study the task scheduling and transmission for improving job processing efficiency in distributed job execution. The problem includes two primary hurdles: how to compute the global job execution order so that the jobs in the system are executed efficiently, and how to assign task transmissions across the system based on the bandwidth constraints to further improve efficiency. We focus on the online setting where jobs arrive dynamically for execution. Our goal is to minimize the average job response time, which is the average time span from every job's arrival to its completion. %We focus on the offline setting where all jobs arrive at the same time for execution. We first present a system model and then define the problem of Workload-Aware Selection with Transmission (WAST). We prove that WAST is NP-hard and propose an algorithm to achieve WAST and analyze its time complexity. Finally, we discuss how to apply WAST to online scenarios where jobs arrive dynamically.
...
\begin{figure*}[htbp]
    ...
    \vspace{0.8em} % 👈 这里是关键：空行代表换行，\vspace 增加上下两行之间的间距
...
%The problem size (the transmission number limit in particular) of the motivating example is relatively small, and hence the performance disparity among different approaches is not substantial. Nevertheless, it reveals some interesting phenomena. %First, the optimal transmission of the task lies in $J_4$ which has the highest priority. This suggests that jobs with higher priorities in scheduling also tend to have high priorities on network resource usage: transferring out a task at the head of the scheduling queue has higher potential of bringing forward the processing start time of more tasks, which raises the probabilities that some jobs could finish earlier (if their completion times are dependent on their tasks at this site).
...
% w_{ij}, & t=0,\\
% w_{ij}^{(t-1)}+l_k,
% & j=\arg\min\limits_{p\in[u_i]} w_{ip}^{(t-1)},\\
% w_{ij}^{(t-1)},
% & j\neq\arg\min\limits_{p\in[u_i]} w_{ip}^{(t-1)},
% \end{cases}\\
% &\forall j\in[u_i].
% \end{aligned}
% \end{equation}
...
t_{ij} + \frac{x_{ij} \cdot r_k}{b_{ij}} + l_k
\le \max_{i \in [m]} f(W_i,y_i).
...
Simulation experiments using read-world job traces are conducted to evaluate the performance of applying our scheduling algorithms to online scenarios as discussed in Section \ref{sec4:deploy}.
...
\label{tab4:average_characteristic}
\end{table}
...
{\includegraphics[width=0.48\textwidth]{Image2/Workload_Ali2018_4_50_GEODIS.eps}%
%\label{fig_second_case}
%\label{Fg:eg1}
}
\hfil
...
%
{\includegraphics[width=0.48\textwidth]{Image2/Workload_Ali2018_4_60_GEODIS.eps}%
%\label{Fg:eg2}
}
\vspace{0.8em}
\subfloat[utilization = 70\%]
%
{\includegraphics[width=0.48\textwidth]{Image2/Workload_Ali2018_4_70_GEODIS.eps}%
%\label{Fg:eg2}
}
\hfil
\subfloat[utilization = 80\%]
%
{\includegraphics[width=0.48\textwidth]{Image2/Workload_Ali2018_4_80_GEODIS.eps}%
%\label{Fg:eg2}
}
...
\centering
\subfloat[utilization = 50\%]
{\includegraphics[width=0.48\textwidth]{Image2/Workload_Ali2017_50.eps}%
%\label{fig_second_case}
%\label{Fg:eg1}
}
\hfil
\subfloat[utilization = 60\%]
%
{\includegraphics[width=0.48\textwidth]{Image2/Workload_Ali2017_60.eps}%
%\label{Fg:eg2}
}
\vspace{0.8em}
\subfloat[utilization = 70\%]
%
{\includegraphics[width=0.48\textwidth]{Image2/Workload_Ali2017_70.eps}%
%\label{Fg:eg2}
}
\hfil
\subfloat[utilization = 80\%]
%
{\includegraphics[width=0.48\textwidth]{Image2/Workload_Ali2017_80.eps}%
%\label{Fg:eg2}
}
...
\centering
\subfloat[utilization = 50\%]
{\includegraphics[width=0.48\textwidth]{Image2/Workload_Ali2018_4_50.eps}%
%\label{fig_second_case}
%\label{Fg:eg1}
}
\hfil
\subfloat[utilization = 60\%]
%
{\includegraphics[width=0.48\textwidth]{Image2/Workload_Ali2018_4_60.eps}%
%\label{Fg:eg2}
}
\vspace{0.8em}
\subfloat[utilization = 70\%]
%
{\includegraphics[width=0.48\textwidth]{Image2/Workload_Ali2018_4_70.eps}%
%\label{Fg:eg2}
}
\hfil
\subfloat[utilization = 80\%]
%
{\includegraphics[width=0.48\textwidth]{Image2/Workload_Ali2018_4_80.eps}%
%\label{Fg:eg2}
}
...
\centering
\subfloat[utilization = 50\%]
{\includegraphics[width=0.48\textwidth]{Image2/CDFpercentile_Ali2017_50.eps}%
% \label{fig:JRT_CDF_a}
}
\hfil
\subfloat[utilization = 60\%]
%
{\includegraphics[width=0.48\textwidth]{Image2/CDFpercentile_Ali2017_60.eps}
% \label{fig:JRT_CDF_b}
}%
\vspace{0.8em}
\subfloat[utilization = 70\%]
%
{\includegraphics[width=0.48\textwidth]{Image2/CDFpercentile_Ali2017_70.eps}}%
\hfil
\subfloat[utilization = 80\%]
%
{\includegraphics[width=0.48\textwidth]{Image2/CDFpercentile_Ali2017_80.eps}
%\label{Fg:eg2}
}
...
\centering
\subfloat[utilization = 50\%]
{\includegraphics[width=0.48\textwidth]{Image2/CDFpercentile_Ali2018_4_50.eps}%
% \label{fig:JRT_CDF_a}
}
\hfil
\subfloat[utilization = 60\%]
%
{\includegraphics[width=0.48\textwidth]{Image2/CDFpercentile_Ali2018_4_60.eps}
% \label{fig:JRT_CDF_b}
}%
\vspace{0.8em}
\subfloat[utilization = 70\%]
%
{\includegraphics[width=0.48\textwidth]{Image2/CDFpercentile_Ali2018_4_70.eps}}%
\hfil
\subfloat[utilization = 80\%]
%
{\includegraphics[width=0.48\textwidth]{Image2/CDFpercentile_Ali2018_4_80.eps}
%\label{Fg:eg2}
}
